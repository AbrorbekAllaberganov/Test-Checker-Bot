...
async def export_group(group_id: int, db: DbDep) -> Response:
    ...
    content = await excel_svc.export_group_excel(db, group_id)
...
async def export_student(student_id: int, db: DbDep) -> Response:
    content = await excel_svc.export_student_excel(db, student_id)
...
async def export_test(test_id: int, db: DbDep) -> Response:
    content = await excel_svc.export_test_excel(db, test_id)
